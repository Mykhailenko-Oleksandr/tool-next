...
// import "react-datepicker/dist/react-datepicker.css"; // стилі календаря
...
// // Кастомний компонент для вибору дати
...
//               {/* Дати через react-datepicker */}
//               <fieldset className={css.fieldset}>
//                 <DateField name="startDate" label="Дата початку" />
//                 <DateField name="endDate" label="Дата завершення" />
//               </fieldset>
...
//         selected={field.value ? new Date(field.value) : null}
//         onChange={(date: Date | null) => {
//           // Зберігаємо в ISO (yyyy-MM-dd) для бекенду/валидації,
//           // але показуємо у форматі dd.MM.yyyy у UI
//           helpers.setValue(date ? date.toISOString().split("T")[0] : "");
//         }}
//         dateFormat="dd.MM.yyyy"
//         locale={uk}
...
//           WEEKDAY_ABBR[nameOfDay.toLowerCase()] || nameOfDay
//         }
//       />
...
function DateField({ name, label, bookedDates = []}: { name: string; label: string; bookedDates?: string[] }) {
  const [field, meta, helpers] = useField(name);

  const excluded = bookedDates.map((d) => new Date(d));

  const formatUkWeekday = (dayName: string) => {
